...
class JunctionScene(QGraphicsScene):
    ...
    def _draw_direction_arrows(self) -> None:
        ...
        for polygon in arrows:
            self.addPolygon(QPolygonF([QPointF(2*polygon.boundingRect().center().x()-p.x(), 2*polygon.boundingRect().center().y()-p.y()) for p in polygon]), no_pen, brush)

    def clear_traffic(self):
        for item in self.traffic_items.values(): self.removeItem(item)
        self.traffic_items.clear()

    def update_traffic(self, engine):
        ...
        for vehicle in engine.vehicles:
            ...
            if item is None:
                item = self.addRect(-2, -2.1, 4, 4.2, QPen(QColor("#FFFFFF"), .5), QBrush(QColor("#2563EB")))
                item.setZValue(8)
                self.traffic_items[vehicle.vehicle_id] = item
            # Map stop/exit progress to the actual painted lines, keeping road scale
            # continuous on either side of the enlarged drawing of the junction.
            def canvas_axis(value):
                magnitude=abs(value)
                scaled=magnitude*13.75 if magnitude<=4 else (55+(magnitude-4)*69/16 if magnitude<=20 else 124+(magnitude-20)*.95)
    ...
    def update_ambulance(self, assessment: GPSAssessment) -> None:
        ...
        def canvas_axis(value):
            ...
            scaled=magnitude*13.75 if magnitude<=4 else (55+(magnitude-4)*69/16 if magnitude<=20 else 124+(magnitude-20)*.95)
            return math.copysign(scaled,value)
        east=math.sin(angle)*assessment.distance_metres
        north=math.cos(angle)*assessment.distance_metres
        target=QPointF(self.CX+canvas_axis(east),self.CY-canvas_axis(north))
